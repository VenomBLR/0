...
class controllerAuth {
  ...
  try {  
    ...
    if(match) {
        ...
        res.setHeader('Authorization', `${token}`); res.status(204).send('Authentication Ok!!!');} else {res.status(404).send("Authentication failed!!!");} 
    });
  ...
  try { 
    ...
    res.setHeader('Authorization', `${newToken}`); res.status(204).send('Password was changed!!!');
    }  catch (error) {res.status(401).send(`Error!!! ${error}`);};
}
}
