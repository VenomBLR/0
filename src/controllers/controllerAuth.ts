...
class controllerAuth {
  ...
  try {  
    ...
    if(match) {
        ...
        res.setHeader('Authorization', `${token}`); res.status(201).send(`${newUser.userid}:${newUser.role}:${token}`);} else {res.status(401).send("Authentication failed!!!");} 
    });
  ...
  try { 
    ...
    if (!(oldPassword && newPassword)) {res.status(401).send("Passwords don't match!!!");}
